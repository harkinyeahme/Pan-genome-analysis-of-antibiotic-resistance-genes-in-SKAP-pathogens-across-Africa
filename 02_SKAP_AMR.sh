...
echo "🔍 Detecting pathogen directories under '$base_project_dir'..."
...
for pathogen_path in "$base_project_dir"/*/; do
    ...
    for country_path in "$pathogen_path"/*/; do
        ...
        if [[ ! -d "$country_path/raw_data" ]]; then
            echo "  Skipping '$country' — no raw_data/ folder found."
            continue
        fi
        ...
        for R1 in "$fastq_files"/*_1.fastq.gz; do
            [[ -e "$R1" ]] || { echo "⚠️  No FASTQ files found in $fastq_files"; break; }
            acc=$(basename "$R1" _1.fastq.gz)
            R2="$fastq_files/${acc}_2.fastq.gz"
            [[ -f "$R2" ]] || { echo "  Missing R2 for $acc — skipping!"; continue; }
        ...
        echo " ✅ Completed processing for country: $country"
    done
